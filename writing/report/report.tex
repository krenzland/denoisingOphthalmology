...
\item We present deep-learning models that are able to reconstruct images corrupted by both mentioned downsampling operators in in real-time for small crop sizes.
...
\begin{figure}[ht]
  ...
  \label{fig:lapdeblur}
  \end{subfigure}
  \caption{Generator architectures.
    ...
    All convolutions except the ones in the resize-convolutions in the \textit{image reconstruction branch} are followed by a leaky rectified linear unit with a negative slope of $0.2$.
    \label{fig:networks}
  }
\end{figure}
We use a modified \textbf{LapSRN} architecture (\cref{fig:lapsrn})~\cite{LapSRN}.
The network utilizes a Laplacian pyramid, it first performs 2x upscaling and then 4x upscaling.
...
We use a depth of 10 per output resolution for this \textit{feature extraction branch}.
...
\item Instead of transposed convolutions we use resize-convolution blocks.
  They are composed of a nearest neighbor interpolation, a reflect-pad of size one and a standard convolution.
  They lead to fewer checkerboard artifacts than transposed convolutions, especially when they are used with adversarial learning~\cite{deconvolution}.
\item We only consider 4x upscaling here, but the approach can be easily extended to larger upscaling factors.
\end{itemize}

For the deblurring case, we modify the architecture (\cref{fig:lapdeblur}) by removing the upscaling blocks from the \textit{feature extraction branch}.
...
In contrast to~\cite{PatchGAN} we use instance normalization~\cite{InstanceNorm} instead of batch normalization layers.
...
An appropriate choice of loss function is important for good reconstructions.
